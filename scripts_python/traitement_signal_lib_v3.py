...
def comparer(schemeToCompare, invite_schema, N_MESURE, cas='client'):
	...
	tab_to_compare = []
	tab_invite = []
	moy_to_compare = sum(schemeToCompare)/len(schemeToCompare)
	moy_invite = sum(invite_schema)/len(invite_schema)
	

	for a in range(N_MESURE):
		if schemeToCompare[a] > moy_to_compare/2:
			tab_to_compare.append(2)
		elif schemeToCompare[a] <moy_to_compare/2 and schemeToCompare[a] > 0:
	...
	for a in range(N_MESURE):
		if invite_schema[a] > moy_invite/2:
			tab_invite.append(2)
		elif invite_schema[a] < moy_invite/2 and invite_schema[a] > 0:
			tab_invite.append(1)
		else:
			tab_invite.append(0)
	if cas == 'client':
